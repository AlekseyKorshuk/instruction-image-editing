...
class LocalBlend:
    def __call__(self, x_t, attention_store):
        maps = [
            m for m in
            attention_store["down_cross"] + attention_store["mid_cross"] + attention_store["up_cross"] if
            m.shape[1] == self.attn_res[0] * self.attn_res[1]
        ]
        maps = [
        ...
        if torch.isnan(mask[0]).any():
            mask[0][torch.isnan(mask[0])] = 0
            tensor_threshold = mask[1]
        elif torch.isnan(mask[1]).any():
            mask[1][torch.isnan(mask[1])] = 0
            tensor_threshold = mask[0]
        else:
            tensor_threshold = mask

        dynamic_threshold = torch.quantile(tensor_threshold, 0.1).item()
        mask = mask.gt(dynamic_threshold)
        mask = mask[:1] + mask[1:]
        ...
        if self.image_mask is not None:
            image_mask_tensor = ToTensor()(self.image_mask.convert("L"))
            image_mask_tensor = image_mask_tensor.unsqueeze(0)  # Add batch dimension if needed
            image_mask_tensor = image_mask_tensor.to(self.device)
            image_mask_tensor = F.interpolate(image_mask_tensor, size=(x_t.shape[2:]), mode='bilinear',
                                              align_corners=False)
            image_mask_tensor = image_mask_tensor > 0.5

            mask = mask & image_mask_tensor
    ...
    def __init__(
            self, prompts: List[str], words: [List[List[str]]], tokenizer, device, attn_res=None,
        ...
        self.alpha_layers = alpha_layers.to(
            device)  # a one-hot vector where the 1s are the words we modify (source and target)
...
def gaussian_kernel(size: int, sigma: float):
    ...
    coords -= size // 2
    distance_sq = coords ** 2
    gaussian_matrix = np.exp(-(distance_sq[:, None] + distance_sq[None, :]) / (2 * sigma ** 2))
    gaussian_matrix /= gaussian_matrix.sum()
    return torch.tensor(gaussian_matrix, dtype=torch.float32)


def blur_tensor_old(input_tensor: torch.Tensor, kernel_size: int, sigma: float):
    """Blurs the input tensor using a Gaussian kernel with the given size and sigma."""
    channels = input_tensor.size(1)
    gaussian_kernel_2d = gaussian_kernel(kernel_size, sigma).unsqueeze(0).unsqueeze(0)
    gaussian_kernel_2d = gaussian_kernel_2d.expand(channels, 1, kernel_size, kernel_size)
    padding = kernel_size // 2
    blurred_tensor = F.conv2d(input_tensor, gaussian_kernel_2d.to(input_tensor.device), padding=padding,
                              groups=channels)
    return blurred_tensor


def blur_tensor(input_tensor, radius):
        ...
        for c in range(channels):
            for y in range(height):
                for x in range(width):
                    if input_tensor[b, c, y, x] == 1:
                        y_min = max(0, y - radius)
                        y_max = min(height, y + radius + 1)
                        x_min = max(0, x - radius)
                        x_max = min(width, x + radius + 1)
                        output_tensor[b, c, y_min:y_max, x_min:x_max] = 1
...
class P2PCrossAttnProcessor:
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        # one line change
        self.controller(
            attention_probs.to(self.controller.device),
            is_cross,
            self.place_in_unet
        )
...
def get_time_words_attention_alpha(
        prompts, num_steps, cross_replace_steps: Union[float, Dict[str, Tuple[float, float]]], tokenizer,
        max_num_words=77
):
...
def highlight_image_differences_with_transparency(img1, img2, diff_color=(0, 0, 255, 127), tolerance=5):
    if img1.mode != 'RGBA':
    ...
    # Compute the absolute difference and check if it's greater than the tolerance
    abs_diff = np.abs(arr1.astype(int) - arr2.astype(int))
    diff = np.any(abs_diff > tolerance, axis=-1)

    highlight = np.copy(arr1)
    for i in range(3):  # Apply RGB colors
        highlight[diff, i] = diff_color[i]
    highlight[diff, 3] = diff_color[3]  # Apply transparency
...
class Prompt2PromptPipeline(StableDiffusionXLPipeline):
    ...
    def __call__(
        ...
        """
        num_blend_steps = int(blend_s * num_inference_steps)
        ...
        self.controller = create_controller(
            prompt, cross_attention_kwargs, num_inference_steps, tokenizer=self.tokenizer, device=device,
        ...
        (
            prompt_embeds_separate,
            negative_prompt_embeds_separate,
            pooled_prompt_embeds_separate,
            negative_pooled_prompt_embeds_separate,
        ) = self.encode_prompt(
            prompt=prompt,
            prompt_2=prompt_2,
            device=device,
        ...
        (
            prompt_embeds_blend,
            negative_prompt_embeds_blend,
            pooled_prompt_embeds_blend,
            negative_pooled_prompt_embeds_blend,
        ) = self.encode_prompt(
        ...
        add_text_embeds_separate = pooled_prompt_embeds_separate
        add_text_embeds_blend = pooled_prompt_embeds_blend
        add_time_ids = self._get_add_time_ids(
            original_size, crops_coords_top_left, target_size, dtype=prompt_embeds_separate.dtype,
        ...
        if negative_original_size is not None and negative_target_size is not None:
            negative_add_time_ids = self._get_add_time_ids(
                ...
                dtype=prompt_embeds_separate.dtype,
            )
        else:
            negative_add_time_ids = add_time_ids

        if do_classifier_free_guidance:
            prompt_embeds_separate = torch.cat([negative_prompt_embeds_separate, prompt_embeds_separate], dim=0)
            prompt_embeds_blend = torch.cat([negative_prompt_embeds_blend, prompt_embeds_blend], dim=0)

            add_text_embeds_separate = torch.cat([negative_pooled_prompt_embeds_separate, add_text_embeds_separate],
                                                 dim=0)
            add_text_embeds_blend = torch.cat([negative_pooled_prompt_embeds_blend, add_text_embeds_blend], dim=0)

            add_time_ids = torch.cat([negative_add_time_ids, add_time_ids], dim=0)

        prompt_embeds_blend = prompt_embeds_blend.to(device)
        add_text_embeds_blend = add_text_embeds_blend.to(device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            # First loop for num_blend_steps
            for i, t in enumerate(timesteps[:num_blend_steps]):

                # expand the latents - in the batch we duplicate the first element for all inputs
                ...
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=prompt_embeds_blend,
                    added_cond_kwargs={"text_embeds": add_text_embeds_blend, "time_ids": add_time_ids}
                ).sample
            ...
            prompt_embeds_separate = prompt_embeds_separate.to(device)
            add_text_embeds_separate = add_text_embeds_separate.to(device)

            for j, t in enumerate(timesteps[num_blend_steps:]):
                ...
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=prompt_embeds_separate,
                    added_cond_kwargs={"text_embeds": add_text_embeds_separate, "time_ids": add_time_ids}
                ).sample
                ...
                # call the callback, if provided
                total_steps_passed = num_blend_steps + j + 1
                if total_steps_passed == num_inference_steps or (
                        (total_steps_passed > num_warmup_steps) and (total_steps_passed % self.scheduler.order == 0)):
                    progress_bar.update()
                    if callback is not None and (total_steps_passed % callback_steps == 0):
                        step_idx = total_steps_passed // getattr(self.scheduler, "order", 1)
                        callback(step_idx, t, latents)
...
def pad_prompts(prompts, pad_token="<|endoftext|>"):
    desired_word_count = max(len(s.split()) for s in prompts)

    padded_strings = []
    for s in prompts:
        word_list = s.split()
        num_words_to_add = desired_word_count - len(word_list)
        padded_string = word_list + [pad_token] * num_words_to_add
        padded_strings.append(' '.join(padded_string))
